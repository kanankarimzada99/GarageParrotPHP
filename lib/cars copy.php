...
function saveCar(PDO $pdo, string $brand, string $model, int $year, int $kilometers, string $gearbox, int $number_doors, float $price, string $color, string $fuel, string $co, string $image, int $id = null): bool
{
  //if id car doesnt exist INSERT
  if ($id === null) {
    $query = $pdo->prepare("INSERT INTO cars ( brand, model, year, kilometers, gearbox, number_doors, price, color, fuel, co, image)"
      . "VALUES(:brand, :model, :year, :kilometers, :gearbox, :number_doors, :price, :color, :fuel, :co, :image)");
  }

  //if id car exist UPDATE
  else {
    $query = $pdo->prepare("UPDATE `cars` SET `brand`= :brand, " . "`model`= :model, " . "`year`= :year," . "`kilometers`= :kilometers," . "`gearbox`= :gearbox, " . "`number_doors`= :number_doors,  " . "`price`= :price, " . "`color`= :color,  " . "`fuel`= :fuel, " . "`co`= :co, " . "`image`= :image WHERE `id`=:id;");
