...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SESSION['token']) && isset($_POST['token']) && $_SESSION['token'] == $_POST['token']) {
  ...
  if (empty($client) && empty($comment) && empty($note)) {
    ...
    echo "<div class='alert alert-danger  m-0' role='alert'>Le nom du client est requis. Seulement caractères et maximum 50 caractères.</div>";
    $errorClient = true;
  } elseif (!preg_match(_REGEX_COMMENT_, $comment)) {
    echo "<div class='alert alert-danger  m-0' role='alert'>Le commentaire du client est requis et doit contenir une longueur maximale de 250 caractères.</div>";
    $errorComment = true;
  }
  ...
  if ($errorEmpty !== true && $errorClient !== true && $errorComment !== true && $errorNote !== true) {
    ...
    if ($res) {
      echo "<div class='alert alert-success  m-0' role='alert mt-3'>L'avis du client a bien été sauvegardé.</div>";
      ...
      echo "<div class='alert alert-danger m-0' role='alert mt-3'>L'avis du client n'a pas été sauvegardé.</div>";
      $errorEmpty = true;
    }
  }
}
