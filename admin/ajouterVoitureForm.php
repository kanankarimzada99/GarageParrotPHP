...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_SESSION['token']) && isset($_POST['token']) && $_SESSION['token'] == $_POST['token']) {
  ...
  //verify if car exist on the database
  foreach ($cars as $car) {
    if ($car['code'] == $code) {
      echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>Code existe déjà.</div>";
      $errorCode = true;
    }
  }
  ...
  if (empty($code) && empty($brand) && empty($model) && empty($year) && empty($price) && empty($kilometer) && empty($color) && empty($gearbox) && empty($doors) && empty($fuel) && empty($co2)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>Vous devez remplir tous les champs.</div>";
    $errorEmpty = true;
  } elseif (!preg_match(_REGEX_CODE_, $code)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>Le code doit contenir uniquement des lettres et chiffres. Trois lettres et trois numéros.</div>";
    $errorCode = true;
  } elseif (!preg_match(_REGEX_BRAND_, $brand)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>La marque doit contenir uniquement des lettres et chiffres et avoir une longueur maximale de 25 caractères.</div>";
    $errorBrand = true;
  } elseif (!preg_match(_REGEX_MODEL_, $model)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>Le modèle doit contenir uniquement des lettres, espaces et chiffres et avoir une longueur maximale de 25 caractères.</div>";
    $errorModel = true;
  } elseif (!preg_match(_REGEX_YEAR_, $year)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>L'anneé doit contenir uniquement des chiffres et avoir une longueur de 4 caractères.</div>";
    $errorYear = true;
  } elseif (!preg_match(_REGEX_PRICE_, $price)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>Le prix doit contenir uniquement des chiffres et avoir une longueur de 4 caractères minimum.</div>";
    $errorPrice = true;
  } elseif (!preg_match(_REGEX_KILOMETERS_, $kilometer)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>La kilométrage doit contenir uniquement des chiffres et avoir une longueur de 3 caractères minimum.</div>";
    $errorKilometers = true;
  } elseif (!preg_match(_REGEX_GEARBOX_, $gearbox)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>La boîte de vitesses doit contenir uniquement des lettres et avoir une longueur maximale de 15 caractères.</div>";
    $errorGearbox = true;
  } elseif (!preg_match(_REGEX_DOORS_, $doors)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>Le numéro de portes doit contenir uniquement des chiffres et avoir une longueur maximale de 2 caractères.</div>";
    $errorDoors = true;
  } elseif (!preg_match(_REGEX_COLOR_, $color)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>La couleur doit contenir uniquement des lettres et espaceset avoir une longueur maximale de 15 caractères.</div>";
    $errorColor = true;
  } elseif (!preg_match(_REGEX_FUEL_, $fuel)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>Le carburant doit contenir uniquement des lettres et avoir une longueur maximale de 15 caractères.</div>";
    $errorFuel = true;
  } elseif (!preg_match(_REGEX_CO2_, $co2)) {
    echo "<div class='alert alert-danger  m-0 mt-3' role='alert'>Le CO2 doit contenir uniquement des chiffres et avoir une longueur maximale de 3 caractères.</div>";
    $errorCO2 = true;
  }
  ...
  if ($errorEmpty !== true && $errorCode !== true && $errorBrand !== true && $errorModel !== true && $errorYear !== true && $errorKilometers !== true && $errorGearbox !== true && $errorDoors !== true && $errorPrice !== true && $errorColor !== true && $errorFuel !== true && $errorCO2 !== true) {
    ...
    if (isset($_FILES['images']['tmp_name']) && $_FILES['images']['tmp_name'] != '') {
      ...
      for ($i = 0; $i < $totalImages; $i++) {
        ...
        $file_type = image_type_to_mime_type(exif_imagetype($_FILES['images']['tmp_name'][$i]));
    ...
    if ($res) {
      echo '<div class="alert alert-success d-inline mt-3  mx-1" role="alert">Le service a bien été sauvegardé.</div>';
    } else {
      echo '<div class="alert alert-danger d-inline mt-3  mx-1" role="alert">Le service n\'a pas été sauvegardé.</div>';
      exit();
    }
  }
}
