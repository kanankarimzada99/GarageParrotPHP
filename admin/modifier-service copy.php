...
$regexDescription = '/^[a-zA-Z0-9\s]{3,200}$/';
  ...
  if ($sizeImage !== false) {
    ...
    if (move_uploaded_file($_FILES['file']['tmp_name'], dirname(__DIR__) . _GARAGE_IMAGES_FOLDER_ . $fileName)) {
      ...
      if (isset($_FILES['file']['name'])) {
        ...
        // var_dump(dirname(__DIR__) . _GARAGE_IMAGES_FOLDER_ . $_FILES['file']['name']);
        //delete old image if new one is uploaded
        unlink(dirname(__DIR__) . _GARAGE_IMAGES_FOLDER_ . $fileName);
      }
    } else {
