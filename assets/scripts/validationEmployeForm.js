$(document).ready(function () {
  ...
  $('#conf-password').on('input', function () {
    checkCpass()
  })
  ...
  $('#submitbtn').click(function () {
    if (
      ...
      $('#form-message').html('')
      var form = $('#addEmploye')[0]
      var data = new FormData(form)
      $.ajax({
        type: 'POST',
        url: 'ajouterEmployeeForm.php',
        data: data,
        processData: false,
        contentType: false,
        cache: false,
...
function checkCpass() {
  ...
  if (cpass == '') {
    $('#cpassword_err').html('Confirm mot de passe ne peut pas être vide.')
    return false
  } else if (pass !== cpass) {
    $('#form-message').hide()
    $('#cpassword_err').html(
      'Confirm mot de passe ne correspond pas à mot de passe'
    )
    return false
  } else {
    $('#cpassword_err').html('')
